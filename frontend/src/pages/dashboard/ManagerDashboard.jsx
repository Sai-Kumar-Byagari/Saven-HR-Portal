...
export default function ManagerDashboard() {
  ...
  const { data: pending, isLoading: l2 } = useQuery({ queryKey: ['leaves','pending'],   queryFn: async () => { const r = await leavesApi.getPending({ limit:1 }); return r.data; } });
  const { data: teamAtt, isLoading: l3 } = useQuery({ queryKey: ['att','team',today],   queryFn: async () => { const r = await attendanceApi.getTeam({ date:today,limit:100 }); return r.data; } });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-base text-gray-400 mt-1">{format(new Date(), 'EEEE, dd MMMM yyyy')} · Your team at a glance</p>
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard title="Team Size"       value={team?.pagination?.total} loading={l1} color="blue"
        ...
        <StatCard title="Present Today"   value={present}                loading={l3} color="green"
          icon={SI(<><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></>)} />
        <StatCard title="On Leave Today"  value={onLeave}                loading={l3} color="yellow"
        ...
        <StatCard title="Leave Approvals" value={pending?.pagination?.total} loading={l2} color="orange"
          icon={SI(<><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></>)} />
      </div>

      <AttendanceBarChart data={weekly} title="Team Weekly Attendance" />
    </div>
  );
}
